      ...
      <li>
        <NavLink
          ...
          className={({ isActive }) =>
            isActive
              ? "active-link-signin"
              : "btn btn-sm btn-outline btn-secondary rounded-full w-full"
          }
          onClick={() => handleSidebarClose()}
        >
          <FaUserShield /> Singin/Up
        </NavLink>
      </li>
    </>
  );
      ...
      <li>
        <div className="dropdown dropdown-bottom md:dropdown-end">
          <label
            ...
            className={`btn btn-sm rounded-full  btn-outline btn-warning text-xl  ${
              isOpenDropdown ? "btn-warning" : ""
            }`}
          >
            <div className="me-3 rounded-full">
              <FaCircleUser />
            </div>
            <span>{isOpenDropdown ? <FaAngleDown /> : <FaAngleUp />}</span>
          </label>
          {isOpenDropdown && (
            <ul
              tabIndex={0}
              onClick={() => setIsOpenDropdown(!isOpenDropdown)}
              className="dropdown-content z-[1] menu-item  shadow bg-white rounded-lg animate-custom mt-4 w-52 "
            >
              <li>
      ...
      <header className="my-container  sticky top-0 z-10">
        <div className="drawer">
          ...
          <div className="drawer-content flex flex-col ">
            ...
            <div className="w-full  navbar min-h-[30px]  bg-violet-900   shadow-lg outline-dashed outline-1 outline-offset-2 outline-violet-900   px-2 rounded-b-3xl ">
              ...
              <div className="md:flex-1">
                <Link
                  to={"/"}
                  className="flex items-center justify-center gap-3 ms-4"
                >
                  <img src={movieIcon} alt="logo" className="w-10 h-10" />
                  <p className="text-xl md:text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-yellow-500 to-white">
                    BD-Cinema House
                  </p>
                </Link>
              </div>
          ...
          <div className="drawer-side z-50 md:hidden">
            ...
            ></label>
            <ul className="menu-item   p-4 w-80 min-h-full bg-zinc-100">
              {/* Sidebar content here */}
              <div>
                <Link
                  to={"/"}
                  className="flex items-center justify-center gap-3 mt-8 mb-6 bg-violet-800 rounded-full outline-2 outline-dashed outline-violet-800 outline-offset-1 p-2"
                  onClick={() => handleSidebarClose()}
                >
                  <img src={movieIcon} alt="logo" className="w-10 h-10" />
                  <p className="text-2xl  font-bold bg-clip-text text-transparent bg-gradient-to-r from-fuchsia-500  to-red-500">
                    BD-Cinema House
                  </p>
                </Link>
              </div>

    ...
    return (
        <>
            ...
            <div className="flex gap-3">
                {
                    movies?.map(movie => <div key={movie._id} onClick={() => { handleScheduleTime(movie._id), setSelectedMovie({_id:movie.movie._id,name:movie.movie.name,image:movie?.movie?.image?.url,category:movie.movie.category,duration:movie.movie.duration,showId:movie._id}) }} className='relative w-1/6'>
                        ...
                        <img src={movie?.movie?.image?.url} className={`${selectedMovie?.showId == movie?._id && 'border border-indigo-500 shadow-md shadow-indigo-400'} rounded-md `} alt="leo" />

...
const TicketBookingSideBar = ({
  ...
  const handlePurchaseTicket = (data) => {
    if (bookingId && selectedSeat?.length == ticketQuantity) {
      axiosInstance
        .patch(`/booking/purchase-confirm/${bookingId}`, data)
        .then((res) => {
    ...
    <div className="md:sticky md:top-20 w-full md:w-80  bg-white outline-dashed outline-1 outline-violet-900 rounded-lg px-4 py-8">
      ...
      <div className="my-4">
        ...
        <div className="flex mt-4">
          <p className="flex items-center gap-3 grow">
            {" "}
            <BsTicketDetailed /> Ticket Quantity
          </p>
        ...
        <div className="flex  mt-4">
          ...
          <p>{selectedSeat?.length <= 0 ? "--" : selectedSeat?.join(",")}</p>
        </div>
        <div className="flex  mt-4">
          <p className="flex items-center gap-3 grow">
            {" "}
            <FaRegMoneyBill1 /> Total Amount
          </p>
          <p>{totalAmount ? `${totalAmount} BDT` : "--"}</p>
        </div>
      </div>
      <form
        onSubmit={handleSubmit(handlePurchaseTicket)}
        className="space-y-3 "
      >
        ...
        <button
          ...
          disabled={!(bookingId && selectedSeat?.length === ticketQuantity)}
        >
          Purchase Ticket
        </button>
      </form>
